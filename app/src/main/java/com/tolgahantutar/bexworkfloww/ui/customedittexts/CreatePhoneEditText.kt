...
class CreatePhoneEditText constructor(
    context: Context,
    editProfileViewModel: EditProfileViewModel,
    view: View,
    action: NavDirections
): TextInputEditText(context){
    val phoneTextInputEditText = this@CreatePhoneEditText
    init {
                    ...
                    MotionEvent.ACTION_UP -> {
                        if (p1.getRawX() >= (phoneTextInputEditText.right - phoneTextInputEditText.compoundDrawables[DRAWABLE_RIGHT].bounds.width())) {
                            if (phoneTextInputEditText.text.toString().isNullOrEmpty()) {
                                phoneTextInputEditText.error = context.getString(R.string.property_cannot_be_null)
                            } else {
                                if (!(PhoneValidation().checkPhoneNumber(phoneTextInputEditText.text.toString()))) {
                                    ...
                                    builder.setTitle(context.getString(R.string.create_phone_operation))
